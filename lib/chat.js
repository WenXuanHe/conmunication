...
class Chat{
    ...
    hander(socket, request){
        socket.on('hander', (msg)=>{
            if(!this.peerIDInfo[msg.peerID]) {

                this.peerIDInfo[msg.peerID]={};
            } 
            //超过最大人数，用P2P连接
            let locationInfo = getLocation(msg.remoteAdress);
            this.peerIDInfo[msg.peerID].locationInfo = locationInfo
        });
    }
    listen(server, app){
        ...
        app.use(async (ctx, next) => {
            const req = ctx.req;
            let remoteAdress = req.connection.remoteAddress;
        });
