...
class Chat{
    ...
    async joinRoom(socket, roomName, io){
        ...
        try{
            let SOCKETID = this.ctx && this.ctx.cookies.get('SOCKETID');
        ...
        socket.on("message", async (message) => {
            try{
                ...
                if(text !=="" ){
                    ...
                    let setChatInfo = await redisServer.setChatInfoByRoom(users.get('roomName'), msgInfo);
                     if(setChatInfo.code){
                         this.chatMessageInfo[users.get('roomName')].push(msgInfo);
                     }
